...
namespace MameOutput_Test
{
	public partial class WndMain : Form
	{
		...
		//Constant for TCP Network and Recoil
		private const int PORT = 8000;
        private const string IPADDRESSSTRING = "127.0.0.1";
		private const int RECOILON = 25;
        ...
        public WndMain(string gameName, List<string> outputsList)
		{
			...
			notifyIcon.Text = "HookOfTheReaperProxy";
										...
										if (splitLine.Length == 2 && !string.IsNullOrEmpty(splitLine[1]) && OutputsList.ContainsKey(splitLine[0]))
										{
                                            ...
                                            if (int.TryParse(splitLine[1], out value))
											{
												if (value >= 0)
												{
													...
													//string signal = _Outputs[OutputsList[splitLine[0]]].Name;
													//string msgCtrl = signal + " = " + splitLine[1] + "\n";
													//this.SendString(msgCtrl);
			...
			while (shouldStopReceiver == false)
			{
				try
				{
					using (NamedPipeServerStream pipeServer = new NamedPipeServerStream("MameHookerProxyRecoilGunA", PipeDirection.In))
					{
                        ...
                        //MessageBox.Show("Connectecd to RecoilGunA Pipe");
                        using (StreamReader reader = new StreamReader(pipeServer, Encoding.UTF8))
						{
							char[] buffer = new char[4096];
							while (true)
							{
								...
								if (_Outputs[1].OutputValue != gunshot)
								{
                                    ...
                                    //string recoilMSG = "GunRecoil_P1 = 1\n";
                                    //this.SendString(recoilMSG);
                                    this.SendValues(_Outputs);
									if (gunshot == 1)
									{
										Thread.Sleep(RECOILON);
										_Outputs[1].OutputValue = 0;
                                        //string noRecoilMSG = "GunRecoil_P1 = 0\n";
                                        //this.SendString(noRecoilMSG);
                                        this.SendValues(_Outputs);
									}
								}
							}
						...
						using (StreamReader reader = new StreamReader(pipeServer, Encoding.UTF8))
						{
							...
							while (true)
							{
								...
								if (_Outputs[2].OutputValue != gunshot)
								{
                                    ...
                                    //string recoilMSG = "GunRecoil_P2 = 1\n";
                                    //this.SendString(recoilMSG);
                                    this.SendValues(_Outputs);
									if (gunshot == 1)
									{
                                        ...
                                        //string noRecoilMSG = "GunRecoil_P2 = 0\n";
                                        //this.SendString(noRecoilMSG);
                                        this.SendValues(_Outputs);
									}
						...
						using (StreamReader reader = new StreamReader(pipeServer, Encoding.UTF8))
						{
							...
							while (true)
							{
								...
								if (_Outputs[3].OutputValue != gunshot)
								{
                                    ...
                                    //string recoilMSG = "GunRecoil_P3 = 1\n";
                                    //this.SendString(recoilMSG);
                                    this.SendValues(_Outputs);
									if (gunshot == 1)
									{
                                        ...
                                        //string noRecoilMSG = "GunRecoil_P3 = 0\n";
                                        //this.SendString(noRecoilMSG);
                                        this.SendValues(_Outputs);
									}
								}
						...
						using (StreamReader reader = new StreamReader(pipeServer, Encoding.UTF8))
						{
							...
							while (true)
							{
								...
								if (_Outputs[4].OutputValue != gunshot)
								{
									...
									//string recoilMSG = "GunRecoil_P4 = 1\n";
									//this.SendString(recoilMSG);
                                    this.SendValues(_Outputs);
									if (gunshot == 1)
									{
                                        ...
                                        //string noRecoilMSG = "GunRecoil_P4 = 0\n";
                                        //this.SendString(noRecoilMSG);
                                        this.SendValues(_Outputs);
									}
								}
